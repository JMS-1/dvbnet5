...
internal class AllocationMap : IEnumerable<AllocationMap.IAllocationInformation>
{
    ...
    public class AllocationPlan(AllocationMap map, IScheduleSource source, SuggestedPlannedTime plan, int index)
    {
        ...
        /// </summary>
        public void Allocate() => m_map.Allocate(m_allocationIndex, m_source, ref m_plan.Planned);
    }
    ...
    private class AllocatedRange : IAllocationInformation
    {
        ...
        /// Gesetzt, wenn keine Zuordnung erfolgt ist.
        /// </summary>
        public bool IsIdle => Sources.Count < 1;
        ...
        public AllocatedRange Clone(DateTime from, DateTime to)
            => new(from, to, m_Map) { Sources = new List<IScheduleSource>(Sources) };
        ...
        public override string ToString()
            => string.Format("{0}[{1:dd.MM.yyyy HH:mm:ss}..{2:dd.MM.yyyy HH:mm:ss}]", m_Map.TotalNumberOfSources - Sources.Count, Start, End);
    }
    ...
    public class AllocationTimeline : TimelineManager<int>
    {
        ...
        protected override int Merge(int existing, int added) => existing + added;
    }
    ...
    /// </summary>
    public bool IsEnabled => TotalNumberOfSources > 0;

    /// <summary>
    /// Gesetzt, wenn keine Planung vorgenommen wurde.
    /// </summary>
    public bool IsEmpty => m_Allocations.Count < 2;
    ...
    {
        // Reset not allowed
        ArgumentOutOfRangeException.ThrowIfEqual(start, DateTime.MinValue);
    ...
    /// </summary>
    public int NumberOfDifferentSources => m_Sources.Count;
    ...
    public override string ToString()
        => string.Format("({0})", string.Join("*", m_Allocations.Select(a => a.ToString()).ToArray()));

    /// <summary>
    /// Meldet, wann die aktuelle Planung beginnt.
    /// </summary>
    public DateTime PlanStart => m_Allocations[0].End;

    /// <summary>
    /// Meldet, wann die aktuelle Planung endet.
    /// </summary>
    public DateTime PlanEnd => m_Allocations[^1].Start;

    /// <summary>
    /// Meldet alle aktiven Einträge.
    /// </summary>
    /// <returns>Die gewünschte Auflistung.</returns>
    public IEnumerator<IAllocationInformation> GetEnumerator() => m_Allocations.GetEnumerator();
