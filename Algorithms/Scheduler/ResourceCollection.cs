    ...
    internal class ResourceCollection : IEnumerable<IScheduleResource>
    {
        ...
        public void Add(IScheduleResource resource)
        {
            ...
            if (resource.Decryption.MaximumParallelSources < 0)
                throw new ArgumentOutOfRangeException("resource", string.Format("Decyption Limit must not be negative but is {0}", resource.Decryption.MaximumParallelSources));
            if (resource.SourceLimit < 0)
                throw new ArgumentOutOfRangeException("resource", string.Format("Source Limit must not be negative but is {0}", resource.SourceLimit));
