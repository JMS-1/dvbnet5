...
public class SourceStreamsManager : IDisposable
{
    ...
    {
        // Validate
        if (m_TransportStream != null) throw new InvalidOperationException();
    ...
    /// </summary>
    public bool? IsDecrypting => m_TransportStream == null ? null : m_Decrypting;
    ...
    /// wurden.
    /// </summary>
    public long? CurrentAudioVideoBytes => m_TransportStream?.AVLength;

    /// <summary>
    /// Meldet die gesamte Anzahl der bisher empfangenen Datenbytes.
    /// </summary>
    public long BytesReceived => BytesBias + (m_TransportStream?.Length ?? 0);

    /// <summary>
    /// Meldet die Anzahl der Datenströme (PID), die für den Empfang benötigt werden.
    /// </summary>
    public int ConsumerCount => m_TransportStream?.StreamCount ?? 0;
    ...
    private static void AddSubtitleInformation(SubtitleInformation subtitle, Dictionary<ushort, List<EPG.SubtitleInfo>> subtitles)
    {
        // Attach to the list
    ...
    /// </summary>
    public FileStreamInformation[] AllFiles => [.. m_AllFiles];
