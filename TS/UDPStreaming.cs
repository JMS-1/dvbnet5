...
public class UDPStreaming(int multicastTTL, int maxQueue)
{
    /// <summary>
    ...
    /// </summary>
    private readonly int m_TTL = multicastTTL;

    /// <summary>
    /// Maximale Anzahl von wartenden Paketen.
    /// </summary>
    private readonly int m_maxQueueLength = maxQueue;
    ...
    {
        // Process all
        for (var i = 0; m_currentConnection.IsActive && (i < buffer.Length);)
            lock (m_queue)
            {
