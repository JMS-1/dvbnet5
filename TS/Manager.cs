...
public class Manager : IDisposable, IStreamConsumer2
{
    ...
    /// </summary>
    public Action<byte[]>? InProcessConsumer;
    ...
    /// </summary>
    public SourceIdentifier? EPGMapping;
    ...
    /// </summary>
    public Manager() : this((Stream)null!)
    {
    }
    ...
    public Manager(Stream target) : this(target, 0)
    {
    }
    ...
    public Manager(string path) : this(path, 0)
    {
    }
    ...
    public Manager(string path, short nextPID) : this(path, nextPID, DefaultBufferSize)
    {
    }
    ...
    public Manager(string path, short nextPID, int bufferSize) : this((Stream)null!, nextPID)
    {
        // Validate
    ...
    public Manager(Stream target, short nextPID)
    {
        ...
        // Create helper
        ProgramAssociation = new Tables.PAT { ProgramStream = NextPID++, ProgramNumber = NextPID++ };
    ...
    {
        // Get the chunks
        var packs = (buffer.Length + PacketSize - 1) / PacketSize;
        var rest = buffer.Length % PacketSize;

        // Forward
        Send(ref counter, pid, buffer, 0, packs, true, (rest == 0) ? PacketSize : rest, true, -1);
    }
    ...
    public VideoStream AddVideo(byte encoding, bool noPCR)
    {
        ...
        // Run
        var pid = AddStream(StreamTypes.Video, encoding, noPCR || forbidPCR, false, null!, null, out bool isPCR);
    ...
    {
        // Run
        var pid = AddStream(StreamTypes.Audio, 255, false, false, null!, aac, out bool isPCR);

        // Set the name of the language
        if (!string.IsNullOrEmpty(name)) ProgramMap.SetAudioLanguage(pid, name);

        // Create
        var audio = new AudioStream(this, pid, isPCR);

        // Remember
        m_Streams.Add(audio);

        // Make it the guide
        if ((m_Splitter != null) && (m_GuidePID == 0)) m_GuidePID = pid;
    ...
    {
        // Run
        var pid = AddStream(StreamTypes.Private, 255, false, false, null!, null, out var isPCR);

        // Set the name of the language
        if (!string.IsNullOrEmpty(name)) ProgramMap.SetAudioLanguage(pid, name);

        // Create
        var dolby = new DolbyStream(this, pid, isPCR);
    ...
    {
        // Run
        var pid = AddStream(StreamTypes.TeleText, 255, false, true, null!, null, out bool isPCR);

        // Create
        var ttx = new TTXStream(this, pid, isPCR);
    ...
    {
        // Run
        var pid = AddStream(StreamTypes.SubTitles, 255, false, true, info, null, out bool isPCR);

        // Create
        var sub = new SubtitleStream(this, pid, isPCR);
    ...
    private short AddStream(StreamTypes type, byte encoding, bool noPCR, bool noPTS, EPG.SubtitleInfo[] info, ushort? aac, out bool isPCR)
    {
        ...
        lock (m_Queue)
        {
            ...
            // Set up
            if (type == StreamTypes.Video)
                buffers.SetAudioVideo(true);
            else if ((type == StreamTypes.Audio) || (type == StreamTypes.Private))
    ...
    void IStreamConsumer.SendPCR(int counter, int pid, long pts)
    {
        ...
        // Split
        var pidh = (byte)(pid >> 8);
        var pidl = (byte)(pid & 0xff);

        // Allocate data
        var ts = new byte[FullSize];
    ...
    public static TimeSpan GetPCR(byte[] packet)
    {
        ...
        // Merge
        var clockTicks = b4 + 2 * (b3 + 256 * (b2 + 256 * (b1 + 256 * b0)));
    ...
    private void Send(ref int counter, int pid, byte[] buffer, int start, int packs, bool isFirst, int sizeOfLast, bool standardPadding, long pts)
    {
        ...
        if ((pts >= 0) && (pid == m_GuidePID) && (m_Splitter != null))
        ...
        // Check mode
        var mustPad = sizeOfLast < PacketSize;

        // Padding mode
        var useSafePadding = mustPad && !standardPadding && (sizeOfLast <= (PacketSize - 2));

        // Calculate
        var end = start + (mustPad ? (packs - 1) : packs) * PacketSize + (mustPad ? sizeOfLast : 0);

        // Validate
        ArgumentOutOfRangeException.ThrowIfGreaterThan(end, buffer.Length, nameof(packs));

        // Allocate data
        var ts = new byte[packs * FullSize];
        ...
        // Fill buffers
        for (var tsi = 0; start < end; start += PacketSize, tsi += PacketSize)
        {
            // Process the header
            ts[tsi++] = 0x47;
            ts[tsi++] = pidh;
    ...
    private void Flush(int pid, bool withPCR)
    {
        ...
        for (var processed = true; processed; processed = false)
        {
    ...
    private void Enqueue(int pid, bool withPCR)
    {
        ...
        {
            // Clone PTS from PES
            collect = new Packet(buffers) { PTS = buffers.PTS };
        }
        else
        {
            // Flush queue
        ...
        // Remember collected data
        if (collect != null) buffers?.Enqueue(collect);
    }
    ...
    {
        // Check mode
        var isTSInfo = (ProgramAssociation.PID == pid) || (ProgramMap.PID == pid) || (ServiceDescription.PID == pid) || (0x12 == pid);
    ...
    public void SetStreamTarget(string client, int port) => m_UDPStream.SetStreamTarget(client, port);

    /// <summary>
    /// Get the TCP/IP client to receive the stream.
    /// </summary>
    public string TCPClient => m_UDPStream.Client;

    /// <summary>
    /// Get the TCP/IP UDP port to send the stream to.
    /// </summary>
    public int TCPPort => m_UDPStream.Port;
    ...
    /// </summary>
    public long ActiveVideoDelay => m_VideoDelay;

    /// <summary>
    /// Meldet die Anzahl der aktiven Datenströme.
    /// </summary>
    public int StreamCount => ProgramMap.Count;
    ...
    public void SetEPGMapping(ushort network, ushort transportStream, ushort service)
        => EPGMapping = new(network, transportStream, service);
