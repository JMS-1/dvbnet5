import * as profileContract from "./IProfileSourceContract";
...
export class ProfileSourcesCache {
  ...
  static getSources(profileName: string): Promise<SourceEntry[]> {
    ...
    if (!promise) {
      // Verwaltung erzeugen.
      ProfileSourcesCache.promises[profileName] = promise = new Promise<
        SourceEntry[]
      >((success) => {
        // Ladevorgang anstossen.
        profileContract
          .getProfileSources(profileName)
          .then((data) =>
            success(data?.map((rawData) => new SourceEntry(rawData)) ?? [])
          );
      });
    }
