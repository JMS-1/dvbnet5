...
export class DayProperty<TDataType>
  extends Property<TDataType, string>
  implements IDaySelector
{
  // Die Namen aller Wochentage, wobei wir die Woche am Montag beginnen lassen.
  readonly dayNames = DateTimeUtils.germanDays.map(
    (d, i) => DateTimeUtils.germanDays[(i + 1) % 7]
  );
  ...
  private set day(newDay: Date) {
    ...
    if (this._pureDate)
      newDay = new Date(
        Date.UTC(newDay.getFullYear(), newDay.getMonth(), newDay.getDate())
      );
