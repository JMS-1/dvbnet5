...
export function uiValue<TValueType>(
  value: TValueType,
  display?: string
): IUiValue<TValueType> {
  return {
...
class SelectableValue<TDataType, TValueType>
  implements ISelectableUiValue<TValueType>
{
  ...
  implements IValueFromList<TValueType>
{
  ...
  constructor(
    ...
    this.addValidator((list) =>
      SingleListProperty.isInList(list as SingleListProperty<unknown>)
    );
  ...
  private static isInList(
    list: SingleListProperty<unknown>
  ): string | undefined {
  ...
  set allowedValues(values: ISelectableUiValue<TValueType>[]) {
    this._allowedValues = (values || []).map(
      (v) => new SelectableValue(v, this)
    );
  ...
  get valueIndex(): number {
    for (let i = 0; i < this.allowedValues.length; i++)
      if (this.allowedValues[i].value === this.value) return i;
