...
export class DateTimeUtils {
    ...
    private static parseHourMinute(hourMinute: string): number | null {
    ...
    private static parseDigit(digit: number): number | null {
        if (digit < 0x30) return null
        if (digit > 0x39) return null
    ...
    static getRealDurationInMinutes(isoStart: string, durationInMinutes: number): number {
        ...
        const end = new Date(
            ...
            start.getDate(),
            start.getHours(),
            start.getMinutes() + durationInMinutes
        )
