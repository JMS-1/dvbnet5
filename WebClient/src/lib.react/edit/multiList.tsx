...
export interface ISelectMultipleFromList
  extends IComponent<IMultiValueFromList<unknown>> {
  // Die Größe der Liste.
  items: number;
}

// React.Js Komponente zur Auswahl von mehreren Elementen aus eine Liste von Elementen.
export class MultiSelect extends ComponentExWithSite<
  IMultiValueFromList<unknown>,
  ISelectMultipleFromList
> {
  ...
  private onChange(ev: React.FormEvent<HTMLSelectElement>): void {
    ...
    for (let i = 0; i < options.length; i++)
      values[i].isSelected = (options[i] as HTMLOptionElement).selected;
  }
}
