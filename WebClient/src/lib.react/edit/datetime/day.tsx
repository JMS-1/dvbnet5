...
export class EditDay extends ComponentWithSite<IDaySelector> {
  ...
  render(): React.JSX.Element {
    return (
      <div
        className="jmslib-editday jmslib-validatable"
        title={this.props.uvm.message}
      >
        <div>
          <Pictogram
            name="prev"
            onClick={() => this.props.uvm.monthBackward.execute()}
          />
          <div>
            <select
              value={this.props.uvm.month}
              onChange={(ev) =>
                (this.props.uvm.month = (ev.target as HTMLSelectElement).value)
              }
            >
            ...
            <select
              ...
              onChange={(ev) =>
                (this.props.uvm.year = (ev.target as HTMLSelectElement).value)
              }
            >
          ...
          <Pictogram
            name="next"
            onClick={() => this.props.uvm.monthForward.execute()}
          />
        </div>
        <table>
          <thead>
  ...
  private getRows(days: ISelectableDay[]): (React.JSX.Element | null)[] {
    ...
    for (let ix = 0; ix < days.length; ix += 7)
