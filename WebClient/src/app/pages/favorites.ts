...
import * as queryContract from "../../web/ISavedGuideQueryContract";
...
export interface IFavoritesPage extends IPage {
  ...
  readonly onlyWithCount: list.IValueFromList<boolean>;
}
...
export class FavoritesPage extends Page implements IFavoritesPage {
  ...
  private static _filter: list.IUiValue<boolean>[] = [
    list.uiValue(true, "Alle Favoriten"),
    list.uiValue(false, "Nur Favoriten mit Sendungen"),
  ];

  // Erlaubt die Einschränklung der Anzeige auf nur die Suchen, zu denen aktuell auch Sendungen in der Programmzeitschrift existieren.
  readonly onlyWithCount = new list.SingleListProperty(
    { value: true },
    "value",
    undefined,
    () => this.refreshUi(),
  ...
  private readFavorites(): Favorite[] {
    return JSON.parse(this.application.profile.guideSearches || "[]").map(
      (e: queryContract.ISavedGuideQueryContract) =>
        new Favorite(
          e,
          (f) => this.show(f),
          (f) => this.remove(f),
  ...
  private remove(favorite: Favorite): Promise<void> {
    ...
    return queryContract
      .updateSearchQueries(favorites.map((f) => f.model))
      .then(() => {
        // Liste aktualisieren.
  ...
  add(favorite: queryContract.ISavedGuideQueryContract): Promise<void> {
    ...
    return queryContract
      .updateSearchQueries(
        [favorite].concat(this.readFavorites().map((f) => f.model))
      )
