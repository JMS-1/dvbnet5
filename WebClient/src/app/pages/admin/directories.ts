...
import * as multiList from "../../../lib/edit/multiList";
...
export interface IAdminDirectoriesPage extends ISection {
  ...
  readonly directories: multiList.IMultiValueFromList<string>;
  ...
  readonly browse: list.IValueFromList<string>;
...
export class DirectoriesSection
  extends Section
  implements IAdminDirectoriesPage
{
  // Der eindeutige Name des Bereichs.
  static readonly route = "directories";

  // Die aktuelle Liste der Aufzeichnungsverzeichnisse.
  readonly directories = new multiList.MultiListProperty(
    {} as { value?: string[] },
    "value",
    undefined,
  ...
  // Eingabe eines Netzwerklaufwerks.
  readonly share = new StringProperty(
    {} as { value?: string },
    "value",
    "Netzwerk-Share",
    () => this.refreshUi()
  ).addValidator(() => this._shareValidation || "");
  ...
  // Die aktuelle Verzeichnisauswahl.
  readonly browse = new list.SingleListProperty(
    {} as { value?: string },
    "value",
    "Server-Verzeichnis",
    () => this.doBrowse()
  ...
  protected loadAsync(): void {
    ...
    contract.getDirectorySettings().then((settings) => {
      ...
      this.directories.allowedValues =
        settings?.targetDirectories.map((d) => list.uiValue(d)) ?? [];
      ...
      contract
        ...
        .then((dirs) => this.setDirectories(dirs ?? []));
    });
  }
  ...
  private setDirectories(directories: string[]): void {
    ...
    this.browse.allowedValues = (directories || []).map((d) =>
      list.uiValue(d, d || "<Bitte auswählen>")
    );
  ...
  private doBrowseUp(): void {
    ...
    if (folder)
      contract
        .browseDirectories(folder, false)
        .then((dirs) => this.setDirectories(dirs ?? []));
  }

  // Ausgewählte Verzeichnisse aus der Liste entfernen.
  private removeDirectories(): void {
    this.directories.allowedValues = this.directories.allowedValues.filter(
      (v) => !v.isSelected
    );
  }
  ...
  protected saveAsync(): Promise<boolean | undefined> {
    ...
    settings.targetDirectories = this.directories.allowedValues.map(
      (v) => v.value
    );
  ...
  private addDirectory(folder: string): void {
    ...
    if (!this.directories.allowedValues.some((v) => v.value === folder))
      this.directories.allowedValues = this.directories.allowedValues.concat([
        list.uiValue(folder),
      ]);
  }
}
