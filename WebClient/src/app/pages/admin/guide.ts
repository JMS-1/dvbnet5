...
import {
  IMultiValueFromList,
  MultiListProperty,
} from "../../../lib/edit/multiList";
import { INumber, NumberProperty } from "../../../lib/edit/number/number";
import {
  getGuideSettings,
  IGuideSettingsContract,
  setGuideSettings,
} from "../../../web/admin/IGuideSettingsContract";
...
export class GuideSection extends Section implements IAdminGuidePage {
  ...
  // Gesetzt, wenn die automatische Aktualisierung der Programmzeitschrift aktiviert wurde.
  readonly isActive = new BooleanProperty(
    {} as { value?: boolean },
    "value",
  ...
  readonly add = new Command(
    ...
    () =>
      !!this.source.value &&
      this.sources.allowedValues.every((v) => v.value !== this.source.value)
  );
  ...
  // Fordert die Konfiguration zur Pflege der Programmzeitschrift an.
  protected loadAsync(): void {
    ...
    getGuideSettings()
      .then((settings) => {
        ...
        this.sources.allowedValues =
          settings?.sources.map((s) => uiValue(s)) ?? [];
  ...
  private removeSources(): void {
    this.sources.allowedValues = this.sources.allowedValues.filter(
      (v) => !v.isSelected
    );
  }
  ...
  private addSource(): void {
    this.sources.allowedValues = this.sources.allowedValues.concat([
      uiValue(this.source.value ?? ""),
    ]);
