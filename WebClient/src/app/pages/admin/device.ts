...
namespace VCRNETClient.App.Admin {
    ...
    export class Device implements IDevice {
        ...
        constructor(
            profile: VCRServer.ProfileContract,
            onChange: () => void,
            private readonly _defaultDevice: () => string
        ) {
            this.name = profile.name
            this.priority = new JMSLib.App.Number(profile, 'priority', undefined, onChange)
                .addRequiredValidator()
                .addMinValidator(0)
                .addMaxValidator(100)
            this.decryption = new JMSLib.App.Number(profile, 'ciLimit', undefined, onChange)
                .addRequiredValidator()
                .addMinValidator(0)
                .addMaxValidator(16)
            this.sources = new JMSLib.App.Number(profile, 'sourceLimit', undefined, onChange)
                .addRequiredValidator()
                .addMinValidator(1)
                .addMaxValidator(32)
            this.active = new JMSLib.App.Flag(profile, 'active', undefined, onChange).addValidator((flag) =>
                this.validateDefaultDevice(flag)
            )
        }

        // Wird ein Gerät nicht verwendet, so darf es auch nicht als Vorgabegerät eingetragen sein.
        private validateDefaultDevice(active: JMSLib.App.Flag): string | undefined {
            if (!active.value)
