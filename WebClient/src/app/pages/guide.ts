...
import * as filterContract from "../../web/IGuideFilterContract";
...
export interface IGuidePage extends IPage, IGuidePageNavigation {
  ...
  readonly sources: list.IValueFromList<string>;

  // Auswahl des Verschlüsselungsfilters.
  readonly encrpytion: list.IValueFromList<guideEncryption>;
  ...
  readonly sourceType: list.IValueFromList<guideSource>;
  ...
  readonly days: list.IValueFromList<string>;

  // Setzt den Anfang der Ergebnisliste auf eine bestimmte Uhrzeit.
  readonly hours: list.IValueFromList<number>;
...
export class GuidePage extends Page implements IGuidePage {
  ...
  private static readonly _cryptOptions = [
    list.uiValue(guideEncryption.Free, "Nur unverschlüsselt"),
    list.uiValue(guideEncryption.Encrypted, "Nur verschlüsselt"),
    list.uiValue(guideEncryption.All, "Alle Quellen"),
  ];

  // Optionen zur Auswahl der Einschränkuzng auf die Art der Quelle.
  private static readonly _typeOptions = [
    list.uiValue(guideSource.Television, "Nur Fernsehen"),
    list.uiValue(guideSource.Radio, "Nur Radio"),
    list.uiValue(guideSource.All, "Alle Quellen"),
  ];

  // Für den Start der aktuellen Ergebnisliste verfügbaren Auswahloptionen für die Uhrzeit.
  private static readonly _hours = [
    list.uiValue(0, "00:00"),
    list.uiValue(6, "06:00"),
    list.uiValue(12, "12:00"),
    list.uiValue(18, "18:00"),
    list.uiValue(20, "20:00"),
    list.uiValue(22, "22:00"),
  ];

  // Die aktuellen Einschränkungen.
  private _filter: filterContract.IGuideFilterContract = {
    contentPattern: "",
    pageIndex: 0,
    pageSize: 20,
    profileName: "",
  ...
  readonly sources = new list.SingleListProperty(
    this._filter,
    "source",
    "Quelle",
    () => this.query()
  ...
  readonly encrpytion = new list.SingleListProperty(
    this._filter,
    "sourceEncryption",
    undefined,
    () => this.query(),
  ...
  readonly days = new list.SingleListProperty(
    this._filter,
    "startISO",
    undefined,
  ...
  readonly hours = new list.SingleListProperty(
    { value: -1 },
    "value",
    undefined,
    () => this.resetIndexAndQuery(),
  ...
  readonly queryString = new StringProperty(
    { value: "" },
    "value",
    "Suche nach",
    () => this.delayedQuery()
  ...
  readonly withContent = new BooleanProperty(
    { value: true },
    "value",
  ...
  readonly resetFilter = new Command(
  ...
  private _jobSelector = new list.SingleListProperty(
    {} as { value?: string },
    "value",
    "zum Auftrag"
  ...
  reset(sections: string[]): void {
    ...
    ProfileCache.getAllProfiles().then((profiles) => {
      ...
      this.profiles.allowedValues = (profiles || []).map((p) =>
        list.uiValue(p.name)
      );
  ...
  findInWeb(model: IGuideItemContract): void {
    window.open(
      `https://www.imdb.com/find/?q=${encodeURIComponent(model.name)}`,
      "_blank"
    );
  }
  ...
  private onDeviceChanged(deviceHasChanged: boolean) {
    ...
    GuideInfoCache.getPromise(this._filter.profileName)
      ...
      .then((jobs) => {
        ...
        const selection = jobs?.map((job) =>
          list.uiValue(job.jobIdentifier, job.name)
        );

        selection?.unshift(list.uiValue("", "(neuen Auftrag anlegen)"));
  ...
  private refreshSources(): void {
    ...
    this.sources.allowedValues = [list.uiValue("", "(Alle Sender)")].concat(
      (this._profileInfo.sourceNames || []).map((s) => list.uiValue(s))
    );
  }
  ...
  private refreshDays(): void {
    ...
    const days = [list.uiValue<string>("", "Jetzt")];
    ...
    if (this._profileInfo.firstStartISO && this._profileInfo.lastStartISO) {
      ...
      for (let i = 0; i < 14 && first.getTime() <= last.getTime(); i++) {
        ...
        days.push(
          list.uiValue(
            start.toISOString(),
            DateTimeUtils.formatShortDate(start)
          )
        );
  ...
  private query(): void {
    ...
    if ((this.hours.value ?? 0) >= 0) {
      ...
      const start = this._filter.startISO
        ? new Date(this._filter.startISO)
        : new Date();
    ...
    this._filter.titlePattern = !query
      ? ""
      : `${this._fulltextQuery ? "*" : "="}${query}`;
    this._filter.contentPattern =
      this.withContent.value && this._fulltextQuery
        ? this._filter.titlePattern
        : "";

    // Auszug aus der Programmzeitschrift abrufen.
    filterContract.queryProgramGuide(this._filter).then((items) => {
  ...
  private createNewSchedule(entry: GuideEntry): void {
    this.application.gotoPage(
      `${this.application.editPage.route};id=*${entry.jobSelector.value};epgid=${entry.id}`
    );
  }
  ...
  private createFavorite(): Promise<void> {
    ...
    const query: ISavedGuideQueryContract = {
      ...
      encryption: this._filter.source
        ? guideEncryption.All
        : this._filter.sourceEncryption,
      source: this._filter.source,
      sourceType: this._filter.source
        ? guideSource.All
        : this._filter.sourceType,
