...
import * as scheduleContract from "../../web/IEditScheduleContract";
import * as jobScheduleContract from "../../web/IJobScheduleDataContract";
...
export class EditPage extends Page implements IEditPage {
    ...
    RecordingDirectoryCache.getPromise()
      ...
      .then((profiles) => {
        ...
        if (sections.length > 0) {
          ...
          return jobScheduleContract.createScheduleFromGuide(id, epgId);
        }
      ...
      .then(
        (info) =>
          info && this.setJobSchedule(info, profileSelection, folderSelection)
      );
  }

  // Erstellt eine neue leere Aufzeichnung.
  private createEmptySchedule(): scheduleContract.IEditScheduleContract {
    const now = new Date(Date.now());

    return <scheduleContract.IEditScheduleContract>{
      allLanguages: !!this.application.profile.languages,
      dolbyDigital: !!this.application.profile.dolby,
      duration: 120,
      dvbSubtitles: !!this.application.profile.subtitles,
  ...
  private onSave(): Promise<void> {
    ...
    const schedule = {
      ...this._jobScheduleInfo?.schedule,
    } as scheduleContract.IEditScheduleContract;

    // Dauer unter Berücksichtigung der Zeitumstellung anpassen.
    schedule.duration = DateTimeUtils.getRealDurationInMinutes(
      schedule.firstStartISO,
      schedule.duration
    );

    // Asynchrone Operation anfordern.
    return jobScheduleContract
      .updateSchedule(
