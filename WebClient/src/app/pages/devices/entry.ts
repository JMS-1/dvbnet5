...
export class Info implements IDeviceInfo {
  ...
  constructor(
    ...
    reload: () => void,
    private readonly _findInGuide: (model: guide.IGuideItemContract) => void,
    private readonly _findInWeb: (model: guide.IGuideItemContract) => void
  ) {
    ...
    if (!_model.isIdle) {
      ...
      this._end = new Date(
        this._start.getTime() + _model.durationInSeconds * 1000
      );
      ...
      if (this.mode === "running")
        this.controller = new Controller(_model, reload);
    }
    ...
    this.showGuide = new BooleanProperty(
      ...
      () =>
        !this._model.hasGuideEntry ||
        !this._model.profileName ||
        !this._model.source ||
        this.mode === "null"
    );
    ...
    guide
      ...
      .then((item) => {
        ...
        this._guideItem = item
          ? new GuideInfo(item, this._findInGuide, this._findInWeb)
          : null;

        // Im Erfolgsfall auch die Zeitschiene aufsetzen.
        if (this._guideItem)
          this._guideTime = new TimeBar(
            this._start,
            this._end,
