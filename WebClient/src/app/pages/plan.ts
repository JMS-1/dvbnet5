...
export interface IPlanPage extends IPage {
  ...
  readonly startFilter: list.IValueFromList<Date>;
...
export class PlanPage extends Page implements IPlanPage {
  ...
  readonly showTasks = new BooleanProperty(
    {} as { value?: boolean },
    "value",
    "Aufgaben einblenden",
    () => this.fireRefresh()
  ...
  readonly startFilter = new list.SingleListProperty(
    {} as { value?: Date },
    "value",
    undefined,
    () => this.fireRefresh(true),
    [] as list.IUiValue<Date>[]
  );
  ...
  reset(sections: string[]): void {
    ...
    const start: list.IUiValue<Date>[] = [];
    ...
    for (let i = 0; i < 7; i++) {
      ...
      start.push(
        list.uiValue(
          now,
          i === 0 ? "Jetzt" : DateTimeUtils.formatShortDate(now)
        )
      );
      ...
      now = new Date(
        now.getFullYear(),
        now.getMonth(),
        now.getDate() + this.application.profile.planDays
      );
    }
  ...
  private query(): void {
    ...
    getPlan(500, endOfTime).then((plan) => {
      ...
      const similiar = this.application.guidePage.findInGuide.bind(
        this.application.guidePage
      );
      const web = this.application.guidePage.findInWeb.bind(
        this.application.guidePage
      );
      ...
      this._jobs =
        plan?.map(
          (job) => new PlanEntry(job, toggleDetail, reload, similiar, web)
        ) ?? [];
