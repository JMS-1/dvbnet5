...
import * as adminSection from "./admin/section";
...
class SectionInfo implements adminSection.ISectionInfoFactory {
...
export interface IAdminPage extends IPage {
  ...
  readonly sections: IValueFromList<adminSection.ISectionInfo>;

  // Erstellt eine Instanz des aktuellen Konfigurationsbereichs.
  getOrCreateCurrentSection(): adminSection.ISection | undefined;
}
...
export class AdminPage extends Page implements IAdminPage {
  ...
  static readonly hoursOfDay: IUiValue<number>[] = [...Array(24)].map(
    (_d: number, i: number) => uiValue(i, DateTimeUtils.formatNumber(i))
  );
  ...
  reset(sections: string[]): void {
    ...
    const curSection =
      allSections.filter((v) => v.value.route === sections[0])[0] ||
      allSections[0];
