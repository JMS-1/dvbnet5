...
export class DurationProperty<TDataType>
  extends Property<TDataType, number>
  implements IDurationEditor
{
  // Beginn (als Uhrzeit).
  readonly startTime;
  ...
  constructor(
    ...
    this.startTime = new TimeProperty(data, propTime, undefined, () =>
      this.onChanged()
    );

    // Aus der aktuellen Startzeit und der aktuellen Dauer das Ende ermitteln.
    const end = new Date(
      new Date(this.startTime.value ?? 0).getTime() + 60000 * (this.value ?? 0)
    );

    // Die Endzeit wird hier als absolute Zeit verwaltet.
    this.endTime = new TimeProperty(
      { value: end.toISOString() },
      "value",
      undefined,
      () => this.onChanged()
    ).addValidator(() => this.checkLimit());
