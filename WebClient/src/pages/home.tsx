  ...
  render(): React.JSX.Element {
    ...
    return (
      <div className="vcrnet-home">
        <div>
          Willkommen zur Benutzeroberfläche des VCR.NET Recording Service. Von
          hier aus geht es direkt zu:
          <ul>
            <li>
          ...
          <ul>
            ...
            <li>
              ...
              , um diese anzusehen, zu verändern, zu reaktivieren oder endgültig
              zu löschen
              <HelpLink page={uvm} topic="archive" />
            </li>
            <li>
              <InternalLink view={application.logPage.route}>
                den Protokollen
              </InternalLink>{" "}
          ...
          <ul>
            <li>
              <InternalLink
                pict="settings"
                view={application.settingsPage.route}
              >
        ...
        <div>
          Je nach den zugeteilten Benutzerrechten können Sie darüber hinaus
          folgende administrative Tätigkeiten wahrnehmen:
          <ul>
            <li>
              {showStartGuide.isReadonly ? (
                ...
                <InternalLink
                  view={() => (showStartGuide.value = !showStartGuide.value)}
                >
                  {showStartGuide.text}
                </InternalLink>
              )}
            ...
            {showStartGuide.value && (
              <Task uvm={uvm.startGuide}>
                Mit der Schaltfläche unter diesem Text kann eine baldmögliche
                Aktualisierung der Programmzeitschrift
                <HelpLink page={uvm} topic="epg" /> angefordert werden. Sind
                gerade Aufzeichnungen aktiv oder in nächster Zeit geplant, so
                wird der VCR.NET Recording Service die Aktualisierung auf den
                nächstmöglichen Zeitpunkt verschieben, da die Ausführung
                regulärer Aufzeichnungen immer Priorität vor allen
                Aktualisierungen hat.
            ...
            <li>
              {showStartScan.isReadonly ? (
                ...
                <InternalLink
                  view={() => (showStartScan.value = !showStartScan.value)}
                >
                  {showStartScan.text}
                </InternalLink>
              )}
            ...
            {showStartScan.value && (
              <Task uvm={uvm.startScan}>
                Hier ist es nun möglich, die Aktualisierung der Quellen der vom
                VCR.NET Recording Service verwalteten DVB.NET Geräte
                anzufordern. Da die Aktualisierung der Quellen eine niedrigere
                Priorität besitzt als die Ausführung regulärer Aufzeichnungen
                kann es sein, dass sie nicht unmittelbar gestartet wird. Der
                VCR.NET Recording Service wird dies aber bei nächster
            ...
            <li>
              ...
              <InternalLink
                view={() => (checkVersion.value = !checkVersion.value)}
              >
                neuere Version
              </InternalLink>{" "}
        ...
        {uvm.isRecording && (
          <div className="vcrnet-warningtext">
            Hinweis: Der VCR.NET Recording Service führt gerade eine oder
            mehrere Aufzeichnungen oder Aktualisierungen von Programmzeitschrift
            respektive Senderliste aus.
            <InternalLink
              pict="info"
              view={uvm.application.devicesPage.route}
            />
          </div>
        )}
        <div>
          Weitere Informationen zum VCR.NET Recording Service findet man hier im
          Bereich der{" "}
          <InternalLink view={`${application.helpPage.route};overview`}>
            Fragen &amp; Antworten
          </InternalLink>
          , auf der{" "}
          <ExternalLink url="http://www.psimarron.net/vcrnet">
            Homepage im Internet
          </ExternalLink>{" "}
          oder im{" "}
          <ExternalLink url="http://jochen.jochen-manns.de/forums/">
            offiziellen Forum
          </ExternalLink>
          .
        </div>
        <div className="vcrnet-home-copyright">
          ...
          <span>Dr. Jochen Manns, 2003-26</span>
        </div>
      </div>
    );
  }
