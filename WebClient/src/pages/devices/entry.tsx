...
// Privater Zustand dieser Komponent.
interface IDeviceState {
  // Gesetzt wenn die LIVE Anzeige aktiv ist.
  showLive?: boolean;
}

// React.Js Komponente zur Anzeige einer Aktivität.
export class Device extends ComponentEx<IDeviceInfo, IDevice, IDeviceState> {
  private readonly openLive = () => this.setState({ showLive: true });

  private readonly closeLive = () => this.setState({ showLive: false });
    ...
    return (
      <tr className="vcrnet-device">
        <td>
          {done ? (
            ...
            showControl.isReadonly ? (
              ...
              <InternalLink
                view={() => (showControl.value = !showControl.value)}
              >
                <Pictogram name={mode} />
              </InternalLink>
            )
        ...
        <td>
          ...
          {this.state?.showLive && (
            <LiveVideo uvm={this.props.uvm} close={this.closeLive} />
          )}
        </td>
