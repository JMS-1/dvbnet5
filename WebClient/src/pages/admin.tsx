...
interface IAdminUiSectionFactory<TSectionType extends ISection> {
  // Der eigentliche Konstruktor.
  new (
    props?: IComponent<TSectionType>,
    context?: IEmpty
  ): AdminSection<TSectionType>;
...
export class Admin extends ComponentWithSite<IAdminPage> {
  ...
  static addSection<TSectionType extends ISection>(
    factory: IAdminUiSectionFactory<TSectionType>
  ): void {
      ...
      <div className="vcrnet-admin">
        <div>
          {this.props.uvm.sections.allowedValues.map((si) => (
            ...
            >
              <InternalLink view={`${this.props.uvm.route};${si.value.route}`}>
                {si.display}
              </InternalLink>
            </div>
          ))}
  ...
  private renderSection(): React.ReactNode {
    ...
    return (
      ...
      React.createElement(factory as any, {
        uvm: this.props.uvm.getOrCreateCurrentSection(),
      })
    );
  }
}
