    ...
    render(page: App.IPage): JSX.Element {
        return (
            <div>
                ...
                <JMSLib.ReactUi.InternalLink view={page.application.devicesPage.route} pict='devices' />, so erlaubt es
                der VCR.NET Recording Service auch, die{' '}
                <JMSLib.ReactUi.InternalLink view={`${page.route};filecontents`}>
                    Aufzeichnungsdateien
                </JMSLib.ReactUi.InternalLink>{' '}
                an eine Netzwerkadresse zu senden. Übertragen wird dabei genau der Inhalt der Aufzeichnungsdatei. Dieses
                Format wird außer von dem{' '}
                <JMSLib.ReactUi.ExternalLink url='http://www.psimarron.net/DVBNETViewer/'>
                    DVB.NET / VCR.NET Viewer
                </JMSLib.ReactUi.ExternalLink>{' '}
                auch von anderen Media Player Anwendungen wie{' '}
                <JMSLib.ReactUi.ExternalLink url='http://www.videolan.org/'>VLC</JMSLib.ReactUi.ExternalLink> verstanden
                und kann zur Anzeige verwendet werden. Bei Benutzung des DVB.NET / VCR.NET Viewers übernimmt dieser alle
                notwendigen Maßnahmen zur Steuerung des Netzwerkversands.
