    ...
    render(page: App.IPage): JSX.Element {
        return (
            <div>
                ...
                zur Programmierung neuer Aufzeichnungen zur Verfügung stellen kann muss erst einmal festgelegt werden,
                mit welchem Inhalt diese zu füllen ist. Im Administrationsbereich der Web Oberfläche gibt es dazu eine
                gesonderten Seite
                <JMSLib.ReactUi.InternalLink view={`${page.application.adminPage.route};guide`} pict='admin' />. Damit
                die Programmzeitschrift überhaupt erstellt wird, muss erst einmal die Sammlung der Informationen an sich
                aktiviert werden.
                ...
                Quellen ausgewählt und zur Liste hinzugefügt werden - natürlich können Quellen auch wieder aus der Liste
                entfernt werden.
                ...
                aufgeweckt. Damit diese Sammlung auch in der Planung der Aufzeichnungen berücksichtigt werden kann, muss
                auch immer eine maximale Laufzeit für die Aktualisierung angegeben werden. Ist diese zu kurz
                eingestellt, so wird der VCR.NET Recording Service die Aktualisierung vorzeitig beenden und die
                Programmzeitschrift ist unvollständig. Der genaue Wert ist abhängig von den ausgewählten Quellen und
                kann nur individuell selbst bestimmt werden. Ist die Aktualisierung doch vorzeitig abgeschlossen, so
                wird auch die Aktivität{' '}
                <JMSLib.ReactUi.InternalLink view={page.application.devicesPage.route} pict='devices' /> auf dem DVB
                Gerät beendet. Mit Hilfe der{' '}
                <JMSLib.ReactUi.InternalLink view={`${page.route};log`}>Protokollliste</JMSLib.ReactUi.InternalLink>{' '}
                lässt sich dann leicht feststellen, was ein typischer Wert für die Laufzeit ist.
                ...
                <JMSLib.ReactUi.InternalLink view={page.application.homePage.route} pict='home' /> anzufordern. Der
                VCR.NET Recording Service wird dann zum nächstmöglichen Zeitpunkt die gewünschte Aktion ausführen. Hier
                gilt wie grundsätzlich für alle Sonderaufgaben: ist eine programmiert Aufzeichnung aktiv oder steht kurz
                bevor, so wird die Aktualisierung verschoben, bis das verwendete DVB Gerät sicher für die konfigurierte
